...
- [ ] Dado `cohort="BBB_2013J"`, quando filtrar, então só aquela
      apresentação sobra e o número de nós removidos vai para
      `meta.stats`.
- [ ] Dada a mesma seed, quando amostrar duas vezes, então a amostra é
      idêntica.
- [ ] Dado `min_weight=2.0`, quando podar, então `validate_projection`
...
- [ ] Dada a rodada completa, quando terminar, então `data/processed`
      ...
      fica verde.
- [ ] Cada redução aplicada está registrada no `meta.json` do artefato —
...
- **Unitários:** determinismo da amostra; corte por peso; núcleo-k.
